    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 sm:py-12">
            ...
            <ul className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((service) => (
                ...
                >
                  ...
                  <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-muted-foreground">{service.MoTa || "Liên hệ gara để được tư vấn chi tiết."}</p>
                  <p className="mono mt-3 text-lg font-bold text-primary">{formatCurrency(service.DonGia)}</p>
                  <button
                    type="button"
                    onClick={onBook}
                    className="mt-5 inline-flex min-h-11 items-center justify-center rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground transition-all hover:bg-primary-hover"
          ...
          <p className="mt-8 rounded-lg border border-info/20 bg-info-soft p-4 text-sm leading-relaxed text-info sm:p-5">
