...
export default function Sidebar({
  ...
  return (
    <aside className={`fixed left-0 top-0 z-30 flex h-screen w-[220px] flex-shrink-0 flex-col bg-[#1e3a6e] text-white transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex items-center border-b border-white/10 px-5 py-4">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-amber-500 shadow-md">
          ...
          <div className="min-w-0">
            <p className="truncate text-xs font-bold leading-tight text-white">{GARAGE_NAME}</p>
            <p className="text-[10px] leading-tight text-white/50">Quản lý gara</p>
          </div>
        </div>
        <button type="button" onClick={onClose} aria-label="Đóng thanh điều hướng" className="ml-auto rounded-lg p-1 text-white/60 hover:bg-white/10 hover:text-white lg:hidden">×</button>
      </div>

      <nav className="flex-1 overflow-y-auto px-2 py-2" aria-label="Điều hướng chính">
        {visibleNav.map((item) => {
          ...
          return (
            <button
              ...
              className={`group relative mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-all ${isActive ? "bg-white/15 text-white" : "text-white/70 hover:bg-white/10 hover:text-white"}`}
            ...
            >
              ...
              <span className={`truncate text-[13px] font-medium ${isActive ? "font-semibold text-white" : ""}`}>{item.label}</span>
              {item.badge && <span className="ml-auto flex-shrink-0 rounded-full bg-amber-500 px-1.5 py-0.5 text-[10px] font-bold text-white">{item.badge}</span>}
            </button>
          );
        })}
      </nav>
