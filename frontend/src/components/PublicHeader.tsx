  ...
  return (
    <>
      <div className="bg-accent text-accent-foreground">
        <div className="mx-auto flex min-h-11 w-full max-w-7xl items-center justify-center gap-6 px-4 sm:justify-between sm:px-6">
          <p className="hidden text-xs font-extrabold uppercase tracking-[0.16em] md:block">
            Đặt lịch trực tuyến · Duyệt báo giá · Theo dõi tiến độ sửa chữa
          </p>
          <a href={`tel:${HOTLINE.replace(/\s/g, "")}`} className="inline-flex min-h-11 items-center gap-2.5 whitespace-nowrap text-sm font-black text-white transition-opacity hover:opacity-80">
            <span className="grid h-7 w-7 place-items-center border border-white/30" aria-hidden="true">{Icons.phone}</span>
            <span className="text-xs font-bold uppercase tracking-[0.12em] text-white/75">Hotline hỗ trợ</span>
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-brand-dark text-white shadow-lg">
        <div className="mx-auto flex h-[72px] w-full max-w-7xl items-center justify-between gap-2 px-4 sm:h-[76px] sm:gap-4 sm:px-6">
          <button type="button" onClick={() => go("home")} aria-label={`${GARAGE_NAME} – Trang chủ`} className="group flex min-w-0 items-center gap-2 text-left sm:gap-3">
            <span className="hidden h-11 w-11 flex-shrink-0 place-items-center bg-accent text-xs font-black tracking-wider text-white transition-all group-hover:bg-white group-hover:text-brand-dark min-[420px]:grid sm:h-12 sm:w-12 sm:text-sm" aria-hidden="true">TC</span>
            <span className="min-w-0">
              <span className="block whitespace-nowrap text-xs font-black uppercase leading-tight tracking-[-0.02em] text-white sm:text-base">Gara Thành Công</span>
          ...
          <nav aria-label="Điều hướng website" className="hidden h-full items-center gap-6 xl:flex 2xl:gap-8">
            {NAV_ITEMS.map((item) => (
