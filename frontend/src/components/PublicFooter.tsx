...
const TRUST_ITEMS = [
  ...
  { title: "Bảo hành trách nhiệm", note: "Hỗ trợ sau bàn giao", icon: Icons.checkCircle },
];
    ...
    <>
      <section aria-labelledby="homepage-cta-title" className="bg-surface-subtle px-4 pb-16 sm:px-6 sm:pb-24">
        <div className="mx-auto grid w-full max-w-7xl overflow-hidden bg-accent text-white lg:grid-cols-[1fr_auto]">
          <div className="p-7 sm:p-10 lg:p-12"><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-white/65">Đừng chờ đến khi xe hỏng nặng</p><h2 id="homepage-cta-title" className="mt-3 max-w-3xl text-2xl font-black uppercase leading-tight tracking-tight sm:text-4xl">Đặt lịch kiểm tra xe trong một phút.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/75">Gara xác nhận giờ hẹn, kiểm tra và báo giá minh bạch trước khi thực hiện.</p></div>
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 px-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
          {TRUST_ITEMS.map((item) => (
            <div key={item.title} className="flex items-center gap-4 border-b border-white/10 py-6 sm:border-r sm:px-5 lg:border-b-0 first:pl-0 last:border-r-0 last:pr-0">
              ...
              <div><p className="text-sm font-extrabold text-white">{item.title}</p><p className="mt-1 text-xs text-white/45">{item.note}</p></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <footer className="bg-brand-dark text-sm text-white/60">
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-4 py-14 sm:grid-cols-2 sm:px-6 lg:grid-cols-[1.4fr_0.8fr_0.8fr_1fr]">
          <div>
            <div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center bg-accent text-sm font-black tracking-wider text-white">TC</span><div><p className="font-black uppercase text-white">{GARAGE_NAME}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.15em] text-white/40">Automotive Service Center</p></div></div>
            <p className="mt-5 max-w-sm text-[13px] leading-6">Đặt lịch, tiếp nhận, báo giá, sửa chữa và bàn giao minh bạch trên một hệ thống thống nhất.</p>
          ...
          <div><p className="public-footer-title">Liên hệ</p><ul className="mt-3 text-[13px]"><li className="flex min-h-11 items-center">123 Lê Lợi, Q.1, TP.HCM</li><li><a href="tel:0901234567" className="inline-flex min-h-11 items-center font-extrabold text-white hover:text-accent">0901 234 567</a></li><li><button type="button" onClick={onBook} className="inline-flex min-h-11 items-center font-bold text-accent hover:text-white">Đặt lịch sửa chữa →</button></li></ul></div>
        </div>
        <div className="border-t border-white/10"><div className="mx-auto flex w-full max-w-7xl flex-col gap-1 px-4 py-5 text-[11px] text-white/35 sm:flex-row sm:items-center sm:justify-between sm:px-6"><p>© 2026 {GARAGE_NAME}. Khóa luận tốt nghiệp — Hệ thống quản lý gara ô tô.</p><p>Dữ liệu demo phục vụ học tập.</p></div></div>
      </footer>
    </>
  );
}
